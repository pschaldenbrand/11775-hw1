    ...
    for idx in xrange(layer_min, layer_max):
        ...
        if config.is_save:
            np.save(os.path.join(config.outpath, '{}tf_fea{}.npy'.format( \
                name, str(idx).zfill(2))), np.squeeze(feature))
            print("Save layer {} with shape {} as {}/{}tf_fea{}.npy".format( \
                    idx, np.squeeze(feature).shape, config.outpath, name, str(idx).zfill(2)))
...
if __name__ == '__main__':
    ...
    param_G = np.load(G_name, allow_pickle=True,encoding = 'latin1').item()
    ...
    if args.phase == 'demo':
        ...
        # Extract Feature
        sound_samples, ids = load_from_txt(args.audio_txt, config=local_config)
    ...
    with tf.Session(config=sess_config) as session:
        ...
        for sound_sample in sound_samples:
            output = extract_feat(model, sound_sample, args, ids[i])
            i += 1
